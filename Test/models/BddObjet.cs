...
namespace Test.Models
{
    public class BddObjet
    {
        ...
        **/
        ...
        public String scriptUpdateCondition(String condition, String key) {
            if(condition != "") {
                return condition;
            }
            ...
            using (NpgsqlCommand command = new NpgsqlCommand())
            {
                ...
                foreach (string col in columnNames)
                {
                    ...
                    if (value != null && !value.ToString().Equals("-1") && !value.ToString().Equals("01/01/0001 00:00:00")) {
                        string updateQuery = $"UPDATE {tableName()} SET {col} = {forQueryValue(value)} WHERE {scriptUpdateCondition(condition,key)}";
